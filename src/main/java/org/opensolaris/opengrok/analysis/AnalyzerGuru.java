...
public class AnalyzerGuru {
     ...
     */
    public Document getDocument(File file, InputStream in, String path, FileAnalyzer fa) throws IOException {
        Document doc = new Document();
        String date = DateTools.timeToString(file.lastModified(), DateTools.Resolution.MILLISECOND);
        doc.add(new Field("u", Util.path2uid(path, date), Field.Store.YES, Field.Index.NOT_ANALYZED));
     ...
     *                             output
     */
    public static void writeXref(FileAnalyzerFactory factory, Reader in, Writer out, Definitions defs, Project project) throws IOException {
        Reader input = in;
    ...
    public static FileAnalyzerFactory find(String file) {
        String path = file;
        int i = 0;
        if (((i = path.lastIndexOf('/')) > 0 || (i = path.lastIndexOf('\\')) > 0) && (i + 1 < path.length())) {
            path = path.substring(i + 1);
        }
        int dotpos = path.lastIndexOf('.');
        if (dotpos >= 0) {
            FileAnalyzerFactory factory = ext.get(path.substring(dotpos + 1).toUpperCase(Locale.getDefault()));
            if (factory != null) {
                return factory;
            }
        }
        ...
        for (Map.Entry<String, byte[]> entry : BOMS.entrySet()) {
            ...
            if (sig.length > bom.length) {
                ...
                if (i == bom.length) {
                    ...
                    return new String(sig, bom.length,                // offset
