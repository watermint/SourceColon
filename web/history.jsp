    ...
    %>
      ...
      %><span class="filelist-hidden"><br/><%
        for (String ifile : files) {
          ...
          if (rev == "") {
      %>
<a class="h" href="<%= context + Prefix.XREF_P + ifile %>"><%= jfile %>
</a><br/><%
        } else {
        %>
<a class="h" href="<%= context + Prefix.XREF_P + ifile %>?r=<%= rev %>"><%= jfile %>
</a><br/><%
            }
          }
        %></span><%
...
<p><b>Note:</b> No associated file changes are available for
  revisions with strike-through numbers (eg.
  <del>1.45</del>
  )
</p>
<%
